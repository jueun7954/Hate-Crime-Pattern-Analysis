...
plot_counterfactual_facet_adjusted <- function(data) {
  ...
  ggplot(data, aes(x = time)) +
    geom_point(aes(y = total_hate_crimes, color = "Observed"), size = 2) +
    geom_line(aes(y = counterfactual, color = "Counterfactual"), linetype = "dashed", size = 1) +
    geom_vline(xintercept = min(data$time[data$intervention == 1]), 
               linetype = "dotted", color = "red") +
    labs(title = "Asian Hate Crimes in Vulnerable States",
         x = "Time (Months)", 
         y = "Total Hate Crimes",
         color = "Legend") +
    ...
    facet_wrap(~ state, scales = "fixed")  # Use fixed y-axis scales
}
